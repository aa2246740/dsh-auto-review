...
export const CREATOR_GRANT_OPERATIONS = ['check', 'activation-plan', 'hot-reload', 'activate-new-client'] as const
...
export interface CreatorGrantBinding {
  ...
  sourceRoot: string
  pluginId: string
  sourceDirectoryIdentity: { dev: string; ino: string }
...
export type CreatorGrantFailure =
  | 'invalid-input' | 'conflict' | 'confirmation-used' | 'capacity' | 'not-found'
  | 'disposed' | 'not-current-owner' | 'unsafe-storage' | 'storage-corrupt'
  | 'external-change' | 'locked' | 'write-failed' | 'clock-invalid'
