...
const SAFE_SESSION_WRITES = new Set([
	"create_goal",
	"todo_write",
	"update_goal"
]);
...
const APPROVE_FOR_ME_SETTINGS_NAMESPACE = settingsNamespace("dsh-approve-for-me");
...
function reviewerModeActive(ctx, agent, settings) {
	return settings.enabled !== false && ctx.permissionPresets.current(agent.session.events) === "approve-for-me";
}
